	...
	private void onUse(PowerUseEvent event) {
		if (event.getPower() == this) {
			...
			if (user.getCooldown(this) <= 0) {
				Damageable target = user.getTargetEntity(Damageable.class, range);
				if (target != null
						&& user.getPlayer().getLocation().distanceSquared(target.getLocation()) >= minRange * minRange
						&& target.isOnGround()) {
