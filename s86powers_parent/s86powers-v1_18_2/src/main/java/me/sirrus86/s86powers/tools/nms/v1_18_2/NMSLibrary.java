package me.sirrus86.s86powers.tools.nms.v1_18_2;
...
//		nmsEntity.setHeadRotation(rider.getEyeLocation().getYaw());
	...
	public int getEntityTypeID(org.bukkit.entity.EntityType type) {
		return IRegistry.W.a(getNMSEntityType(type));
	}
...
//		return types;
	...
	public void removePathfinding(Creature entity) {
		try {
			...
			handle.bQ = new PathfinderGoalSelector(handle.s.ac());
			handle.bR = new PathfinderGoalSelector(handle.s.ac());
...
//			}
	...
	public org.bukkit.inventory.ItemStack setItemGlow(org.bukkit.inventory.ItemStack item) {
		...
		NBTTagCompound tag = nmsItem.s() ? nmsItem.t() : new NBTTagCompound();
	...
	public void setTamed(Creature entity, Player player) {
		try {
			entity.setTarget(null);
			EntityCreature handle = ((CraftCreature)entity).getHandle();
			PathfinderGoalSelector goalSelector = handle.bQ,
					targetSelector = handle.bR;
	...
	public void unTame(Creature entity) {
		try {
			EntityCreature handle = ((CraftCreature)entity).getHandle();
			PathfinderGoalSelector[] goalSelectors = { handle.bQ, handle.bR };
