...
//		return types;
	...
	public void removePathfinding(Creature entity) {
		try {
			...
			handle.bO = new PathfinderGoalSelector(handle.t.getMethodProfilerSupplier());
			handle.bP = new PathfinderGoalSelector(handle.t.getMethodProfilerSupplier()); // TODO: Make separate for 1.17.1
...
//			}
	...
	public void setTamed(Creature entity, Player player) {
		try {
			entity.setTarget(null);
			EntityCreature handle = ((CraftCreature)entity).getHandle();
			PathfinderGoalSelector goalSelector = handle.bO,
					targetSelector = handle.bP;
	...
	public void unTame(Creature entity) {
		try {
			EntityCreature handle = ((CraftCreature)entity).getHandle();
			PathfinderGoalSelector[] goalSelectors = { handle.bO, handle.bP };
